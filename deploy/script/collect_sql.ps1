...
$DRDir = Split-Path -Parent $ScriptDir
Write-Host "工作目录: $DRDir"
...
try {
    ...
    foreach ($dir in $SrcDirs) {
        ...
        foreach ($dbType in $DbTypes) {
            ...
            # 将绝对路径转换为相对路径
            $relativePath = $initSql.Substring($DRDir.Length + 1)
